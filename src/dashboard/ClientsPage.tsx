...
import { useClients, newId } from "./store";
...
const ClientsPage = () => {
  ...
  const [form, setForm] = useState({ name: "", whatsapp: "", discord: "", service: "", status: "ativo" as ClientStatus });
  ...
  const add = () => {
    ...
    setClients((prev) => [{ id: newId(), ...form }, ...prev]);
  ...
  return (
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        <EmptyState title="Nenhum cliente encontrado" description="Adicione seu primeiro cliente clicando em 'Novo cliente'." />
      ) : (
          ...
          {filtered.map((c) => (
            <div key={c.id} className="card-dark p-5 flex flex-col gap-3 hover:border-primary/50 transition-colors">
              <div className="flex items-start justify-between gap-3">
                ...
                <StatusPill tone={tone(c.status)}>{statusOptions.find((s) => s.value === c.status)?.label}</StatusPill>
              </div>

              <div className="text-xs text-muted-foreground space-y-1">
                {c.whatsapp && <p>WhatsApp: <span className="text-foreground/80">{c.whatsapp}</span></p>}
                {c.discord && <p>Discord: <span className="text-foreground/80">{c.discord}</span></p>}
              </div>
